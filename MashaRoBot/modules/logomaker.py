...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./MashaRoBot/resources/LOGOS/*")
    ...
    fname = "Cutiepii.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Made by @PrinceSerenaBot")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report @NobisukiSupport, {e}')
